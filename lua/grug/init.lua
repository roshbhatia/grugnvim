...
require('lazy').setup({{
    ...
    opts = {}
}, 'jackguo380/vim-lsp-cxx-highlight', 'liuchengxu/vim-which-key', 'mhinz/vim-startify', 'gorbit99/codewindow.nvim',
'gelguy/wilder.nvim', 'bluz71/nvim-linefly',
...
-- Load Exile plugins
local exile_modules = {'general', 'barline', 'startify', 'nvim-tree', 'codewindow', 'wilder'}
...
-- Kubectl keybindings
vim.api.nvim_set_keymap('n', '<leader>kl', ':lua require("kubectl").get_logs()<CR>', {
    noremap = true,
    silent = true
})
vim.api.nvim_set_keymap('n', '<leader>kp', ':lua require("kubectl").get_pods()<CR>', {
    noremap = true,
    silent = true
})
vim.api.nvim_set_keymap('n', '<leader>kd', ':lua require("kubectl").describe()<CR>', {
    noremap = true,
    silent = true
})
